                                     ...
                                     struct demucs_model_4s *model)
{
    ...
    // load weights
    {
        ...
        // seek past one uint32_t values in the beginning of the file
        // to skip the magic
                                 ...
                                 int32_t nelements)
{
    ...
    // Create a temporary MatrixXh to load the data
    Eigen::MatrixXh tmp_matrix(matrix.rows(), matrix.cols());

    // loading quantized weights
    const size_t bpe = sizeof(Eigen::half);
    auto nbytes_tensor = tmp_matrix.size() * bpe;

    fread(tmp_matrix.data(), bpe, nelements, f);

    printf("%16s: [%5d, %5d], type = float, %6.2f MB\n", name.data(), ne[0],
           ne[1], nbytes_tensor / 1024.0 / 1024.0);

    // Cast the temporary MatrixXh to MatrixXf and swap layout
    matrix = tmp_matrix.cast<float>();
                                   ...
                                   int32_t nelements)
{
    ...
    // Create a temporary Tensor4dXh to load the data
    Eigen::Tensor4dXh tmp_tensor(tensor.dimensions());
    ...
    printf("%16s: [%5d, %5d, %5d, %5d], type = float, %6.2f MB\n", name.data(),
           ne[0], ne[1], ne[2], ne[3], nbytes_tensor / 1024.0 / 1024.0);

    // Create a column-major Tensor4dXf with the same dimensions as tmp_tensor
    Eigen::Tensor4dXf tensor_cm(ne[0], ne[1], ne[2], ne[3]);
    ...
    for (int i = 0; i < ne[0]; ++i)
    {
        for (int j = 0; j < ne[1]; ++j)
        {
            for (int k = 0; k < ne[2]; ++k)
            {
                ...
                {
                    tensor_cm(i, j, k, l) =
                        static_cast<float>(tmp_tensor(i, j, k, l));
                }
            }
        }
    }
                                   ...
                                   int32_t nelements)
{
    if (tensor.size() != nelements)
    {
        fprintf(stderr, "%s: tensor '%s' has wrong size in model file\n",
                __func__, name.data());
        fprintf(stderr,
                "%s: model file shape: [%d], demucs.cpp shape: "
                "[%d]\n",
                __func__, ne[0], (int)tensor.dimension(0));
        return 0;
    }

    // Create a temporary Tensor1dXh to load the data
    Eigen::Tensor1dXh tmp_tensor(ne[0]);
    ...
    printf("%16s: [%5d], type = float, %6.2f MB\n", name.data(), ne[0],
           nbytes_tensor / 1024.0 / 1024.0);

    // Cast the temporary Tensor1dXh to Tensor1dXf
    tensor = tmp_tensor.cast<float>().swap_layout();
                                 ...
                                 int32_t nelements)
{
    ...
    // loading weights
    const size_t bpe = sizeof(Eigen::half);
    auto nbytes_tensor = tmp_vector.size() * bpe;

    fread(tmp_vector.data(), bpe, nelements, f);

    printf("%16s: [%5d], type = float, %6.2f MB\n", name.data(), ne[0],
           nbytes_tensor / 1024.0 / 1024.0);

    // Cast the temporary VectorXh to VectorXf
    vector = tmp_vector.cast<float>();
