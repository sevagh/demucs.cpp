...
void demucscpp_v3::lstm_reset_zero(const int encoder_idx,
                                   const int dconv_idx,
                                   struct demucscpp_v3::demucs_v3_segment_buffers &buffers)
{
    for (int lstm_layer = 0; lstm_layer < 2; ++lstm_layer)
    {
        for (int direction = 0; direction < 2; ++direction)
        {
            ...
            buffers.lstm_hidden[encoder_idx][dconv_idx][lstm_layer][direction].setZero();
            buffers.lstm_cell[encoder_idx][dconv_idx][lstm_layer][direction].setZero();

            // Assuming lstm_output_per_direction and lstm_output are structured similarly
            // and need to be reset as well. Adjust dimensions as necessary.
            buffers.lstm_output_per_direction[encoder_idx][dconv_idx][lstm_layer][direction].setZero();
        }
        ...
        for (int direction = 0; direction < 2; ++direction)
        {
                 ...
                 t += (direction == 0 ? 1 : -1))
            {
                Eigen::MatrixXf gates =
                    model.encoder_4_5_dconv_layers_3_lstm_ih_w[encoder_idx][dconv_idx][lstm_layer][direction] * loop_input.row(t).transpose() +
                    model.encoder_4_5_dconv_layers_3_lstm_ih_b[encoder_idx][dconv_idx][lstm_layer][direction] +
                    model.encoder_4_5_dconv_layers_3_lstm_hh_w[encoder_idx][dconv_idx][lstm_layer][direction] * buffers.lstm_hidden[encoder_idx][dconv_idx][lstm_layer][direction] +
                ...
                Eigen::MatrixXf f_t = sigmoid(gates.block(hidden_state_size, 0, hidden_state_size, 1));
                Eigen::MatrixXf g_t = gates.block(2 * hidden_state_size, 0, hidden_state_size, 1).array().tanh();
                Eigen::MatrixXf o_t = sigmoid(gates.block(3 * hidden_state_size, 0, hidden_state_size, 1));

                Eigen::MatrixXf c_t = f_t.array() * buffers.lstm_cell[encoder_idx][dconv_idx][lstm_layer][direction].array() + i_t.array() * g_t.array();
                Eigen::MatrixXf h_t = o_t.array() * c_t.array().tanh();

                buffers.lstm_hidden[encoder_idx][dconv_idx][lstm_layer][direction] = h_t;
                buffers.lstm_cell[encoder_idx][dconv_idx][lstm_layer][direction] = c_t;

                buffers.lstm_output_per_direction[encoder_idx][dconv_idx][lstm_layer][direction].row(t) = h_t.transpose(); // Adjusted for transposed output
            }
        }

        // Concatenate the outputs from both directions
        buffers.lstm_output[encoder_idx][dconv_idx][lstm_layer] << buffers.lstm_output_per_direction[encoder_idx][dconv_idx][lstm_layer][0],
         buffers.lstm_output_per_direction[encoder_idx][dconv_idx][lstm_layer][1];
