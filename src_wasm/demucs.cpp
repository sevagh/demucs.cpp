...
extern "C"
{
    ...
    void modelInit(char *model_data, int model_data_size)
    {
        ...
        bool success =
            load_demucs_model_for_wasm(model_data, model_data_size, &model);
        if (!success)
        {
    ...
    void modelDemixSegment(const float *left, const float *right, int length,
                           ...
                           float *right_4, float *left_5, float *right_5,
                           float *left_6, float *right_6,
                           bool batch_mode_param)
    {
        ...
        const int total_inferences = 1;
        int current_inference = 0;

        demucscpp::ProgressCallback progressCallback =
            [batch_mode_param, total_inferences, &current_inference](float increment, std::string msg)
        {
            float global_progress = ((float)current_inference + increment) / total_inferences;
            std::cout << msg << std::endl;
            sendProgressUpdate(global_progress, batch_mode_param);
        };
